class OfficeExpense < ApplicationRecord
  ...
  after_save :send_expense_notify

  def set_month_date
    self.spent_month = self.spent_date.month
    self.spent_year = self.spent_date.year
  end
  ...
  def send_expense_notify
    # FinanceMailer.notify_expense(self).deliver_now
    begin 
      ...
      # amt_received_office = @office_incomes.where(transaction_via_id: transaction_via()"OFFICE")
      ...
      @balance_sheet = BalanceSheet.find_by(month_year: self.spent_date.beginning_of_month)
  ...
  def self.to_csv(options = {})
    CSV.generate(options) do |csv|
      csv << ["Transaction Via", "Spent Date", "Money Spent", "Description"]
      all.each do |expense|
        csv << [expense.transaction_via.name, expense.spent_date.strftime("%b %d"), expense.money_spent, expense.description]
      end
    end
  end
  ...
  def transaction_via_amount(name)
    TransactionVium.find_by(name: name)
  end
