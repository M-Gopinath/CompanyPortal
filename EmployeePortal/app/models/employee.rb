class Employee < ApplicationRecord
  ...
  def approver?
    Employee.find_by(employee_id: self.supervisor_id)
  end
  ...
  def user_details
    unless self.admin?
      if self.ctc.present?
        medical = 4000
        ctc_without_medical = self.ctc - medical

        basic = ((ctc_without_medical.to_f * 30)/100)
        hra = ((basic.to_f*50)/100)
        pf = 21600
        # pf = ((basic*12)/100)
        gratuity = ((basic.to_f*4.82).to_f/100)
        incentive = generate_insentive
        special = (ctc_without_medical - (basic + hra + pf + gratuity + medical + incentive))
        employee_ctc = self.build_employee_ctc_year(basic_pay: basic, hra: hra, pf_employeer_contribution: pf, gratuity_employeer_contribution: gratuity, medical_insurance: medical, special_allowance: special, incentive: incentive)
        employee_ctc.save
      end
