 class PrivacyPoliciesController < ApplicationController
  ...
  def update
  	respond_to do |format|
  		if @privacy_policy.update(privacy_policy_params)
  			format.html { redirect_to privacy_policies_path, notice: 'Privacy Policy was successfully updated.' }
  ...
  def update_policy
    @privacy_policy = PrivacyPolicy.find_by(id: params[:policy_id])
    if @privacy_policy.present?
      @privacy_policy.update(privacy_policy_params)
      redirect_to privacy_policies_path
    end
  end
  ...
  private
    ...
    def privacy_policy_params
    	params.require(:privacy_policy).permit(:topic, :description, :for_client)
    end
  end
